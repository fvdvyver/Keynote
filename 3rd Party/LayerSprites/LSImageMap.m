...
- (NSString *)LS_normalizedPathWithDefaultExtension:(NSString *)extension
{
    ...
    {
        // **********************************************************************************
        // ******** Mushroom Cloud Change ***************************************************
        typedef NSString * (*send_type)(void *, SEL, NSString *);
        send_type function = (send_type)objc_msgSend;
        
        return function((__bridge void *)(fileManager), normalizedPathSelector, path);
        // End of change
        // **********************************************************************************
    }
...
- (UIImage *)decompressedImageWithfile:(NSString *)file;
    ...
    if (dataPath && [[dataPath pathExtension] isEqualToString:@"atlasc"])
    {
        if ((self = [self init]))
        {
            ...
            for (NSDictionary *dict in atlas[@"images"])
            {
                ...
                // **********************************************************************************
                // ******** Mushroom Cloud Change ***************************************************
                UIImage *image = [self decompressedImageWithfile:imagePath];
                // End of change
                // **********************************************************************************
    ...
    if (dict && [dict isKindOfClass:[NSDictionary class]])
    {
        if (!image)
        {
            ...
            if (metadata)
            {
                ...
                // **********************************************************************************
                // ******** Mushroom Cloud Change ***************************************************
                image = [self decompressedImageWithfile:[path LS_normalizedPathWithDefaultExtension:@"png"]];
                // End of change
                // **********************************************************************************
...
- (UIImage *)decompressedImageWithfile:(NSString *)file
{
    ...
    //redraw image using device context to make sure it is decompressed
